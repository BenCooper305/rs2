...
class RawGoalNode: public rclcpp::Node
{
    ...
    private:
            ...
            std::shared_ptr<std_srvs::srv::Trigger::Response> response)
        {
            (void)request;
            publishRawPoints();
            response->message = "sending points";
            response->success = true;
        }

        void publishRawPoints()
        {
            for(size_t i = 0; i != Mysegments_.size(); i++)//modify for data
            {
                std::vector<MyPoint> currentSeg = Mysegments_[i];//modify for data
        ...
        std::vector<MyPoint> face_points = {
            {2.15, 0.6, 0}, {3.2, 4.45, 0}, {3.2, 0.85, 0}, {3.75, 1.75, 0}, {2.85, 4.65, 0},
            {2.5, 4.75, 0}, {3.5, 4.15, 0}, {1.1, 2.25, 0}, {1.5, 4.15, 0}, {1.0, 2.75, 0},
            {3.5, 1.25, 0}, {1.25, 3.75, 0}, {3.75, 3.75, 0}, {1.1, 3.25, 0}, {2.5, 0.5, 0},
            {1.8, 0.85, 0}, {3.9, 2.25, 0}, {4.0, 2.75, 0}, {1.8, 4.45, 0}, {2.15, 4.65, 0},
            {1.25, 1.75, 0}, {3.9, 3.25, 0}, {2.85, 0.6, 0}, {1.5, 1.25, 0}
        };
