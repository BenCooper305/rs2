...
private: 
    ...
    void detectAndProcessEyes(const cv::Mat& upperFace, int newX, int newY, int newWidth, int newHeight); // faceROIadj
...
void PointPublisher::detectAndProcessEyes(const cv::Mat& upperFace, int newX, int newY, int newWidth, int newHeight) {
    ...
    for (const cv::Rect& eye : eyes_) {
        ...
        cv::Mat eyeROI = upperFace(cv::Rect(eyeX + newX, eyeY + newY, eyeWidth, eyeHeight)); //  faceROIadj // _ newX + newY
        cv::Mat eyeEdges;
...
int PointPublisher::image_processing(cv::Mat image){
    ...
    for (const cv::Rect& face : faces_) {
        ...
        cv::Mat upperFace = gray_(cv::Rect(x, y, w2, h2/2));
        ...
        detectAndProcessEyes(upperFace, newX, newY, newWidth, newHeight); // faceROIadj
